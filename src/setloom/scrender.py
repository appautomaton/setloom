...
"""Headless vibe-stem rendering for the Spec 5 vertical slice.
...
score.recordNRT(
    ...
    action: {{ File.delete("{osc_path}"); "RENDER-DONE {part}".postln; 0.exit }}
);
)
"""
...
def render_part_stem(
    ...
    if part == "lead":
        ...
        render_jobs = []
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(family_scores)) as executor:
            ...
            for family, family_stem, future in render_jobs:
                future.result()
        ...
        subprocess.run(
            ["sox", str(raw_bus), str(out_wav), "gain", "-n", "-4"],
            check=True,
            capture_output=True,
        )
        raw_bus.unlink()
...
def default_render_jobs() -> int:
    """Conservative default: use available cores without spawning unbounded sclang."""
    cores = os.cpu_count() or 4
    return max(1, min(len(VIBE_PARTS), cores - 2))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--jobs",
        type=int,
        default=default_render_jobs(),
        help="parallel stem render jobs (default: cores-2, capped by stem count)",
    )
